...
folder_path <- "D:/Kerjaan/Project Reformatting Data/Palimanan file 2.2"
...
for (mdb_file in mdb_files) {
  ...
  summary.frame <- matrix(NA, ncol = 5, nrow = length(axles))
  ...
  for (i in seq_along(axles)) {
    ...
    summary.frame[i, 4] <- as.numeric(mean(subset_data$SPEED, na.rm = TRUE))
    summary.frame[i, 5] <- as.numeric(max(subset_data$SPEED, na.rm = TRUE))
...
saveWorkbook(wb, file = "Summary File Palimanan file 2.2.xlsx")
cat("Tabel telah diekspor ke", "Summary File Palimanan file 2.2.xlsx", "\n")
